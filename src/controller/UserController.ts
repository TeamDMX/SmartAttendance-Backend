...
export class UserController {
	...
	static async save(data) {
		...
		try {
			if (data.lecCode != "null") {
				lecturer = await getRepository(Lecturer).findOne({
					where: { code: data.lecCode },
				});
				if (!lecturer) {
					throw {
						status: false,
				...
				student = await getRepository(Student).findOne({
					where: { regNumber: data.stuRegNumber },
				});
				if (!student) {
					throw {
						status: false,
	...
	static async update(userId: number, data) {
		...
		const selectedEntry = await getRepository(User)
		...
		// hash passwrod
		editedEntry.password = crypto
			.createHash("sha512")
			.update(editedEntry.password)
			.digest("hex");
	...
	static async delete(userId: number) {
		...
		const entry = await getRepository(User)
			.findOne({ id: userId })
			.catch((e) => {
				console.log(e.code, e);
				throw {
